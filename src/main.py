...
class Sprite:
    ...
    def collides_with(self, target: "Sprite") -> bool:
    ...
    def draw_hitbox(self, ctx: js.CanvasRenderingContext2D) -> None:
        """Draw the hitbox of the sprite for debugging."""
        if self.visible:
            ctx.save()
...
class World:
    ...
    def create_world(self) -> None:
        ...
        for i in range(5):
            tree = Sprite(100 + i * 150, 300, 30, 80, "black", "tree")
            self.add_sprite(tree)

        # Add some rocks (gray circles - drawn as squares for now)
        for i in range(3):
            rock = Sprite(200 + i * 200, 400, 25, 25, "gray", "rock")
            self.add_sprite(rock)

        # Add a house (brown rectangle with red roof)
        house_base = Sprite(400, 250, 100, 60, "brown", "house_base")
        house_roof = Sprite(380, 230, 140, 30, "red", "house_roof")
...
class Player(Sprite):
    ...
    def handle_input(self) -> None:
        ...
        # Prevent player from moving through sprites
        for sprite in world.sprites:
            # Let house sprites be the goal of the level
            if sprite.name == "house_base" or sprite.name == "house_roof":
                continue
